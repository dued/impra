...
class WizardFacturaRegVentas(models.TransientModel):
	_name = "factura.regventas"    
	...
	def action_excel(self):
		...
		self.env.cr.execute((""" SELECT a.date_invoice as fec_emi, case a.tipo_doc when 'FAC' then '01' when 'BOL' then '03' when 'NCR' then '07' when 'NDB' then '08' else 'XX' end td, a.serie serie, a.numeracion numero, 
							(case when state='anulado' then 'ANULADO' else coalesce(c.vat,'') end)::varchar ruc, 
							(case when state='anulado' then 'ANULADO' else coalesce(c.name,'') end)::varchar raz_social,
							...
							''::varchar m_percepcion, a.amount_total, round(1/rcr.rate,4) tc,
							coalesce(tdoc_ref, substring(correlativo_fisico,1,2))::varchar tipodoc_afecta,
							coalesce(fec_ref, a.fecha_fisico) fecdoc_afecta,
							coalesce(serie_ref, lpad(replace(substring(correlativo_fisico,4,3),'_',''),3,'0'))::varchar seriedoc_afecta ,
							coalesce(num_ref, lpad(replace(substring(correlativo_fisico,8),'_',''),8,'0'))::varchar numdoc_afecta
							...
							left join (select id, case tipo_doc 
								...
								on (ref.id = a.refund_invoice_id) 
							left join res_currency b on(b.id = a.currency_id)
							left join res_partner c on(c.id = a.partner_id)
							left join res_currency_rate rcr on (rcr.name = a.date_invoice and rcr.id = 3)
							where extract(year from a.date_invoice)::text || lpad(extract(month from a.date_invoice)::text,2,'0') ='%s' and a.state not in('draft') and substring(a.type, 1,3) ='out'
		...
		for item in oconsulta:
			...
			if not item[16]:
				fecha_null = '1900-01-01'
				#date_object = datetime.strptime(fecha_null, '%Y-%m-%d').date()
				ws.cell(i,17).value ='' # date_object
