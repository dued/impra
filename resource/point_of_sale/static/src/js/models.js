...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        model:  'res.company',
        fields: [ 'currency_id', 'email', 'website', 'company_registry', 'vat', 'name', 'phone', 'partner_id' , 'country_id', 'tax_calculation_rounding_method', 'ocultar_logo', 'texto_logo','rubro_empresa','imprimir_comprobante'],
        ...
        fields: ['name','street','city','state_id','country_id','vat',
                 ...
                 'property_account_position_id','property_product_pricelist','provincia_id','distrito_id', 'tipo_documento'],
        ...
        fields: ['name','street','city','state_id','country_id','vat',
                 ...
                 'property_account_position_id','property_product_pricelist','provincia_id','distrito_id', 'tipo_documento'],
        domain: [['id','=',1]],
        loaded: function(self,partner_company){
            self.company_partner = partner_company;
        },
    },{
        model:  'res.country',
        ...
        fields: ['name', 'code'],
        domain: [['country_id','=',173]],
        loaded: function(self,states){
            self.states = states;
        },
    },{
        model:  'res.provincia',
        fields: ['name', 'code', 'departamento_id'],
        order: _.map(['name'], function (name) { return {name: name}; }),
        loaded: function(self,provincias){
            self.provincias = provincias;
        },
    },{
        model:  'res.distrito',
        fields: ['name', 'code', 'provincia_id'],
        order: _.map(['name'], function (name) { return {name: name}; }),
        loaded: function(self,distritos){
        ...
        fields: ['display_name', 'list_price', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_id', 'taxes_id',
                 ...
                 'product_tmpl_id','tracking', 'um_sunat'],
        ...
        model:  'um.sunat',
        fields: ['name', 'codigo_um', 'abreviatura'],
        loaded: function(self, um_sunat){
            self.um_sunat = um_sunat;
        }
    },{
    ...
    // Retornar la abreviatura de la unidad de medida SUNAT.
    get_umsunat: function(um_sun){
        var self = this;
        var abr = "";
        for (var i = 0, len = self.um_sunat.length; i<len; i++){
            if (self.um_sunat[i].id === um_sun){
                //abr = self.um_sunat[i].codigo_um;
                if (self.um_sunat[i].abreviatura == ""){
                    abr = self.um_sunat[i].codigo_um;
                }else{
                    abr = self.um_sunat[i].abreviatura;
                }
    ...
    get_partners: function(){
        ...
        var def  = new $.Deferred();
        var fields = _.find(this.models,function(model){ return model.model === 'res.partner'; }).fields;
        var domain = [['category_id.name','=','mesero']];
        var loaded = rpc.query({
            ...
            .then(function(partners){
                partner_list.push("Ninguno");
                partner_list_id.push(0);
                for (var i = 0, len=partners.length; i < len; i++){                    
                    partner_list.push(partners[i].name);
                    partner_list_id.push(partners[i].id);                  
                    
                }
                if (partner_list.length > 0){
                    for (var i = 0, len=partner_list.length; i < len; i++){
                        var content = $('#partner-select').html();
                        var new_option = '<option value=' + partner_list_id[i] + '>' + partner_list[i] + '</option\n';
                        $('#partner-select').html(content +  new_option);
                    }
                    $('#partner-select').selectedIndex = 0 ;
                    $('#ocultar_funcion').hide();
    ...
    captura_nombre_vendedor: function() {        
        $('#mivendedor').html($('#partner-select option:selected').text());                    
    },
    ...
    push_and_invoice_order: function(order){
        ...
        //se agrego una condicion mas de && order.is_to_invoice() para que el cliente defecto sea de la configuracion solo cuando es boleta  
        if(!order.get_client() && order.is_to_invoice()){
        ...
        this.flush_mutex.exec(function(){
            ...
            var fac_bol =order.is_to_invoice();
            ...
            var transfer = self._flush_orders([self.db.get_order(order_id)], {timeout:30000, to_invoice: fac_bol});
            ...
            transfer.pipe(function(order_server_id){
                ...
                //}}).done(function () {
                self.db.set_order_server_id(order_server_id);
                invoiced.resolve();
                done.resolve();
                //});
            });
...
exports.Paymentline = Backbone.Model.extend({
    ...
    //prueba envio observacion
    set_observacion: function(value){        
        this.observacion = value
        this.trigger('change',this);
    },   
    get_observacion: function(){
        return this.observacion;
    },
    //termina prueba envio observacion
    ...
    export_as_JSON: function(){
        return {
            ...
            observacion: this.get_observacion()
        };
    },
...
exports.Order = Backbone.Model.extend({
    initialize: function(attributes,options){
        ...
        this.to_invoice     = false;   //variable modificada extra
        this.to_invoice2     = true; //agrego esta variable extra
    ...
    set_vendedor_id: function(){        
                
        this.vendedor_id = $('#partner-select').val();
    },

    get_vendedor_id: function(){
        return this.vendedor_id;
    },

    set_guia_nro: function(){        
                
        this.guia_nro = $('#nro_guia').val();
    },

    get_guia_nro: function(){
        return this.guia_nro;
    },

    set_placa_nro: function(){        
                
        this.placa_nro = $('#nro_placa').val();
    },

    get_placa_nro: function(){
        return this.placa_nro;
    },

    set_pdf_b64: function(dato){        
                
        this.pdf_b64 = dato;
    },

    get_pdf_b64: function(){
        return this.pdf_b64;
    },
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            vendedor_id: this.get_vendedor_id(),
            guia_nro: this.get_guia_nro(),
            placa_nro: this.get_placa_nro(),
        };
    },
    export_for_printing: function(){
        var orderlines = [];
    ...
    //extra
    set_to_invoice2: function(to_invoice2) {
    this.assert_editable();
        this.to_invoice2 = to_invoice2;
    },
    is_to_invoice2: function(){
            return this.to_invoice2;
    },
