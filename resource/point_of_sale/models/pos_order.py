...
class PosOrder(models.Model):
    ...
    def _order_fields(self, ui_order):
        ...
        #_logger.debug(ui_order.get('vendedor_id'))
        #_logger.info(ui_order.get('vendedor_id') + " VENDEDOR ")

        if ui_order.get('vendedor_id') == 0:
        	IdVendedor = False
        else:
        	IdVendedor = ui_order.get('vendedor_id')
        ...
        return {
            ...
            'vendedor_id': False,
            'guia_nro': ui_order['guia_nro'],
            'placa_nro': ui_order['placa_nro'],
        }
    ...
    def _payment_fields(self, ui_paymentline):
        ...
        return {
            ...
            'observacion': ui_paymentline.get('observacion',''),
        }
    ...
    def _process_order(self, pos_order):
        ...
        partner_id = pos_order['partner_id'] or False   

        if partner_id == False:
            partner_id = pos_session.config_id.cliente_default.id
            pos_order['partner_id'] = partner_id                         
            _logger.info(str(partner_id) + " esto es mi mensaje nuevo ahora ")
    ...
    def _prepare_invoice(self,xto_invoice):
        """
        ...
        if xto_invoice==True:
            ...
            fac_bol=self.session_id.config_id.invoice_journal_id.id
        else:
            fac_bol=self.session_id.config_id.boleta_journal_id.id
    ...
    vendedor_id = fields.Many2one('res.partner', string='Vendedor', change_default=True, index=True, states={'draft': [('readonly', False)], 'paid': [('readonly', False)]})
    guia_nro = fields.Char(string = 'Nro Guía')
    placa_nro = fields.Char(string = 'Nro Placa')
    ...
    @api.multi
    def action_pos_order_invoice(self,xto_invoice):
        ...
        for order in self:
            ...
            invoice = Invoice.new(order._prepare_invoice(xto_invoice))
    ...
    #prueba de funcion obtener serie factura de account_invoice
    @api.model
    def buscar_factura(self,dato):
        
        invoice = self.env['account.invoice']        

        atrapa_id_busqueda=self.search([('id','=',int(dato[0]))]).invoice_id.id
        informacion=invoice.browse(atrapa_id_busqueda)

        tipo_document = informacion.tipo_doc
        texto_factura = ""
        if tipo_document[0:3] == "FAC":
            texto_factura="FACTURA ELECTRONICA"
        elif tipo_document[0:3] == "BOL":
            texto_factura="BOLETA DE VENTA ELECTRONICA"
        numero_cpe = informacion.serie + "-" + informacion.numeracion

        cadena=str(informacion.imagen_qr)
        qr_imagen=cadena[2:len(cadena)-1]

        cad_logo = str(informacion.company_id.logo)
        logo_imagem = cad_logo[2:len(cad_logo)-1]

        #para traer el cliente de facturacion
        cliente_name="Cliente: " + informacion.partner_id.name
        
        #para traer la pagina de custodia
        pagina_custodia= informacion.company_id.pag_custodia

        if informacion.company_id.imprimir_comprobante == True:

            reporte = self.env['reporte.crystalreport']
            report_pdf = reporte.search([('id_registro_modelo','=',atrapa_id_busqueda)], limit=1)
            cadena_pdf = str(report_pdf.txt_binary)
            pdf_reporte = cadena_pdf[2:len(cadena_pdf)-1]
            
            #return {'txt_factura': texto_factura,'numeracion_factura': numero_cpe,'qrcode': qr_imagen,'cliente': cliente_name, 'pag_custodia': pagina_custodia, 'logo_emp': logo_imagem, 'pdf_reporte': pdf_reporte}
        else:
            pdf_reporte =""
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            # Esta línea se ha comentado ya que todo es facturado.
            #if to_invoice:
            ...
            #if to_invoice:
            pos_order.action_pos_order_invoice(to_invoice)
    ...
    def _prepare_bank_statement_line_payment_values(self, data):
        ...
        args = {
            ...
            'observacion':  data.get('observacion','hola'),
        }
